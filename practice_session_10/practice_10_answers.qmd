...
## Question 1 : Ch^2 test - What Type of Ice Cream?
...
4.) Find the p-value using `pchisq()` function. *hint*: find the function in help tab. Calculate now the
...
So we find strong evidence that the flavors are not equally preferred. Note that the strongest
...
### Question 2: Chi^2 test- Genetic Variation
...
2.) There are three categories so we have $d f=2$. Using a chi-square distribution with
$df=2$.
...
The F-stat is 2.798 and it is the same as the previous F-stat found in Q1.
...
```
 ...
 he p-value is 0.0439 which is the same as the p-value found in Q1. (The answers may vary due to the randomization).
